...
- GitOps bundle chart testing for the Argo CD engine: an Argo matrix entry (detected from a rendered `argoproj.io` `Application`, or explicit) installs Argo CD Core from the pinned `argoproj/argo-cd` release (vendored in `container-gitops/` and bundled at `/etc/ats/gitops/argo.yaml`), switches on applications-in-any-namespace, applies a permissive `default` `AppProject`, and registers the Giant Swarm catalog as an OCI Helm repository. The bundle then deploys, converges (every `Application` `Healthy` and `Synced`), tests and drains through the same per-engine flow as Flux. Example bundle chart in `examples/apps/argo-bundle-app`.
