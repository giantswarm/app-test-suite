...
- `integration` test label and `IntegrationTestScenario`: tests marked `@pytest.mark.integration` (pytest) or `//go:build integration` (Go) now run as a distinct scenario between functional and upgrade.
