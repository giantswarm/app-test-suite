...
- GitOps bundle chart testing for the Flux engine: when a scenario has Flux matrix entries (detected or explicit), ATS installs Flux on the test cluster (source, kustomize and helm controllers from the pinned `fluxcd/flux2` release, vendored in `container-gitops/` and bundled at `/etc/ats/gitops/flux.yaml`), deploys the bundle into a per-engine namespace (`<deploy-namespace>-<engine>`) stacking the engine values overlay, waits until every Flux `HelmRelease`/`Kustomization`/`OCIRepository` is ready and the resource set is stable (nested bundles converge), runs the test suite (with `ATS_EXTRA_GITOPS_ENGINE` set), and after teardown waits for the entry's GitOps resources to drain. Readiness timeouts dump the conditions of every non-ready resource. The upgrade scenario falls back to the plain Helm deploy for now. Example bundle chart in `examples/apps/flux-bundle-app`.
