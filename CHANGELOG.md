...
- GitOps bundle chart testing for the Flux engine: when a scenario has Flux matrix entries (detected or explicit), ATS installs Flux on the test cluster (source, kustomize and helm controllers from the pinned `fluxcd/flux2` release, vendored in `container-gitops/` and bundled at `/etc/ats/gitops/flux.yaml`), deploys the bundle into a per-engine namespace (`<deploy-namespace>-<engine>`) stacking the engine values overlay, waits until every Flux `HelmRelease`/`Kustomization`/`OCIRepository` is ready and the resource set is stable (nested bundles converge), runs the test suite (with `ATS_EXTRA_GITOPS_ENGINE` set), and after teardown waits for the entry's GitOps resources to drain. Readiness timeouts dump the conditions of every non-ready resource. Example bundle chart in `examples/apps/flux-bundle-app`.
- GitOps bundle chart testing for the Argo CD engine: an Argo matrix entry (detected from a rendered `argoproj.io` `Application`, or explicit) installs Argo CD Core from the pinned `argoproj/argo-cd` release (vendored in `container-gitops/` and bundled at `/etc/ats/gitops/argo.yaml`), switches on applications-in-any-namespace, applies a permissive `default` `AppProject`, registers the Giant Swarm catalog as an OCI Helm repository, and registers health overrides for Gateway API resources (`HTTPRoute`/`Gateway`/…) so a route with no controller-written status counts as ready on a controller-less test cluster while an explicit rejection still surfaces as `Degraded`. The bundle then deploys, converges (every `Application` `Healthy` and `Synced`), tests and drains through the same per-engine flow as Flux. Example bundle chart in `examples/apps/argo-bundle-app`.
- GitOps bundle chart testing now covers the upgrade scenario: the stable release and the upgrade to the version under test are both deployed into the per-engine namespace with the engine values overlay stacked, and ATS waits for the bundle to become ready after each deploy, so the stable-to-candidate CR-set transition runs under a live engine. Engine selection (`auto`/`flux`/`helm`) and the readiness timeout use the same `--upgrade-tests-gitops-*` options as the other scenarios.
- GitOps bundle chart testing config surface: per-test-type `--<test>-tests-gitops-engines` (`auto` by default: detect Flux/Argo resources from the rendered chart; `helm` forces the plain Helm deploy; explicit `flux`/`argo` lists supported), `--<test>-tests-gitops-values-<engine>` overlay options (with the `ci/gitops-values-<engine>.yaml` convention) and `--<test>-tests-gitops-bundle-ready-timeout`, plus global `--gitops-flux-install-manifest` / `--gitops-argo-install-manifest` install-source pins. Detection runs on every scenario; engine-aware deployment lands in a follow-up.
