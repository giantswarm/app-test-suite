...
- The CRD bootstrap now waits for every CRD on the test cluster to be `Established` (polled through the
  API, 120 s) after the `kubectl apply --server-side` of `--cluster-crds`. `kubectl apply` returns as soon
  as the API server has accepted the objects, before it serves the new kinds, so a chart whose templates
  render one of those kinds (a kagent `Agent` for a CRD passed via `--cluster-crds`) failed
  `helm upgrade --install` with `no matches for kind` when the deploy started a few milliseconds later, and
  passed on a rerun against the same cluster. The wait polls the API instead of running `kubectl wait`: in
  that window a CRD has no `.status` yet, and both `--for=condition=Established` and `--for=jsonpath=...`
  exit 1 on the missing field instead of waiting for it.
...
- `--cluster-crds`: path or URL passed to `kubectl apply --server-side -f` to bootstrap CRDs on the test cluster. Defaults to `/etc/ats/crds` (unchanged for Docker image runs); set it to point `ats` at a local CRD bundle when running as a standalone `uv tool` outside the container.

### Changed

- **BREAKING:** `ats` no longer creates or destroys clusters. Cluster provisioning is out of scope: you always provide a `kubeconfig` for an existing cluster to run the tests on. The built-in `kind` cluster provider (which ran `kind create/delete cluster`) and the whole pluggable cluster-provider abstraction have been removed, along with the per-test-type `--{smoke,functional,upgrade}-tests-cluster-type` / `--{smoke,functional,upgrade}-tests-cluster-config-file` selectors, the `--external-cluster-kubeconfig-path` / `--external-cluster-type` / `--external-cluster-version` options, and the `--kind-cluster-image` / `--kind-cluster-image-override` options. They are replaced by three top-level options that apply to all test scenarios: `--cluster-kubeconfig` (required; path to the kubeconfig), `--cluster-type` (optional free-text label, exported as `ATS_CLUSTER_TYPE`), and `--cluster-version` (optional free-text label, exported as `ATS_CLUSTER_VERSION`). The `kind` and `docker` binaries and the Docker-socket / `DOCKER_GID` requirement have been dropped from the Docker image. **Migration:** create your cluster yourself (for example `kind create cluster && kind get kubeconfig > ./kube.config`) and replace the removed options with `--cluster-kubeconfig ./kube.config` (plus optional `--cluster-type` / `--cluster-version`). Unlike the old external provider, the type/version labels are now optional. CI consumers using `architect/run-tests-with-ats` must move to an orb version that passes the new options.
...
- `--steps` now works: selecting specific steps (e.g. `--steps smoke`) no longer crashes the selected test scenario. The chart info extraction step was filtered out together with the unselected scenarios, leaving the scenario without the chart name/version it needs; it now always runs regardless of the `--steps`/`--skip-steps` selection ([giantswarm#19871](https://github.com/giantswarm/giantswarm/issues/19871)).
...
- `uv sync` no longer fails with `Permission denied` on `/ats/.cache/uv` when the Docker image is run as a mapped non-root UID/GID matching the image default (`1000:1000`); the cache directory is now created with the correct ownership at build time.
