...
class SimpleTestScenario(BuildStep, ABC):
    ...
    def _ensure_app_platform_ready(self, kube_config_path: str) -> None:
        ...
        logger.info(
            f"Running {self._APPTESTCTL_BIN} tool to ensure app platform components on the target cluster"
        )
        run_res = run_and_log(
            args
        )  # nosec, file is either autogenerated or in user's responsibility
        if run_res.returncode != 0:
            raise ATSTestError(
                "Bootstrapping app platform on the target cluster failed"
            )
    ...
    def pre_run(self, config: argparse.Namespace) -> None:
        ...
        # verify version
        run_res = run_and_log(
            [self._APPTESTCTL_BIN, "version"], capture_output=True
        )  # nosec
        ...
        cluster_type = ClusterType(
            get_config_value_by_cmd_line_option(
                config, self._config_cluster_type_attribute_name
            )
        )
        ...
        self._configured_cluster_config_file = (
            cluster_config_file if cluster_config_file is not None else ""
        )
    ...
    def _deploy_tested_chart_as_app(
        self, config: argparse.Namespace, context: Context
    ) -> None:
    ...
    def _deploy_chart(
        ...
        logger.info(
            f"Deploying App CR '{app_name}' into '{deploy_namespace}' namespace."
        )
        app_obj = create_app(
            self._kube_client,
            app_name,
    ...
    def _upload_chart_to_app_catalog(
        self, config: argparse.Namespace, chart_file_path: str
    ) -> None:
        ...
        ChartMuseumAppRepository(self._kube_client).upload_artifact(
