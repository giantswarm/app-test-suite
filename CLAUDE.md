...
**app-test-suite (ATS)** is a Python CLI tool for testing Helm chart apps on the Giant Swarm App Platform. It orchestrates Kubernetes clusters, deploys charts, and runs test suites (pytest or Go tests) against them. It runs inside a Docker container that bundles kubectl, kind, docker CLI, helm, ct (chart-testing), and Go.
...
- **Dockerfile** uses multi-stage builds; binary versions (kubectl, kind, helm, ct, Go) are pinned as build args. CRDs are vendored in `container-crds/` (see its README) and copied to `/etc/ats/crds/`
