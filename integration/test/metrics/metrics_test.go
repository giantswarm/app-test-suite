...
func TestMetrics(t *testing.T) {
	...
	var podName string
	{
		logger.Debugf(ctx, "waiting for %#q pod", projectName)

		podName, err = waitForPod(ctx)
		if err != nil {
			t.Fatalf("could not get %#q pod %#v", projectName, err)
		}

		logger.Debugf(ctx, "waited for %#q pod", projectName)
	}
	...
	{
		err = appTest.CtrlClient().Get(
			ctx,
			types.NamespacedName{Name: projectName, Namespace: namespace},
			&app)
		if err != nil {
			t.Fatalf("could not get %#q app %#v", projectName, err)
		}
...
func waitForPod(ctx context.Context) (string, error) {
	...
	o := func() error {
		...
		pods, err := appTest.K8sClient().CoreV1().Pods(namespace).List(ctx, lo)
		if err != nil {
